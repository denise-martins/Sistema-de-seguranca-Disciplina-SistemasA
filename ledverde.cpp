...
void setup() {
  ...
  pinMode(led, OUTPUT);
  pinMode(ledSenhaOK, OUTPUT);   // <<< LED NOVO
...
void loop() {
  ...
  for (int i = 0; i < 4; i++) {
    if (digitalRead(botoes[i]) == HIGH) {
      ...
      if (indice == 4) {
        ...
        if (correta) {
          // ---- SENHA CORRETA ----
          digitalWrite(ledSenhaOK, HIGH);  // <<< LIGA LED DE SUCESSO

          meuServo.write(180);  
          delay(2000);           // trava aberta por 2 segundos
          meuServo.write(0);     // volta ao normal

          digitalWrite(ledSenhaOK, LOW);   // <<< DESLIGA LED
        }
  ...
  // Se nada apertado, apaga o LED principal
